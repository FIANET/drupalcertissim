...
class CertissimXMLElement extends DOMElement
{
	...
	/**
	 * creates an CertissimXMLElement then adds as a child then returns the child
	 * 
	 * @param string $name
	 * @param string $value
	...
	{
		$child = $this->appendChild(new CertissimXMLElement($name));
		if ($cdata && constant('CERTISSIM_USE_CDATA') !== false)
