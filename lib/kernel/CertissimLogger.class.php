...
class CertissimLogger
{
	...
	private static function openHandle()
	{
		...
		if (!file_exists(CERTISSIM_ROOT_DIR.'/logs/'))
			if (!mkdir(CERTISSIM_ROOT_DIR.'/logs/'))
				die('Error creating logs folder');

		$log_filename = CERTISSIM_ROOT_DIR.'/logs/fianet_log.txt';
		...
		{
			$prefix = CERTISSIM_ROOT_DIR.'/logs/fianetlog-';
			$base = date('YmdHis');
			$sufix = '.txt';
	 ...
	 */
	public static function insertLogCertissim($from, $msg)
	{
