...
class CertissimSocket {
  ...
  public function __construct($url, $method = 'POST', array $data = null) {
    ...
    if (strtoupper($method) == 'GET' || strtoupper($method) == 'POST')
      ...
      $msg = "La mÃ©thode demandÃ©e ($method) n'est pas reconnue.";
      insertLogCertissim(__METHOD__ . ' : ' . __LINE__, $msg);
   ...
   * nettoie l'url appelÃ©e pour sÃ©parer hÃ´te et script
   *
   * @param string $url url du script appelÃ©
   */
  ...
  function build_header() {
    if ($this->method == 'POST') {
      ...
      if (strlen($this->path . $this->data) > 2048) {
        insertLogCertissim(get_class($this) . " : __construct", "Maximum length in get method reached(" . strlen($this->path . $this->data) . ")");
      }
  ...
  function connect($header) {
   //connects with SSL protocol if secure connection asked, HTTP protocol otherwise
    if ($this->is_ssl){
        $socket = fsockopen('tls://' . $this->host, $this->port, $this->errno, $this->errstr, CertissimSocket::TIMEOUT);
    if($socket == false){
        insertLogCertissim(__METHOD__ . ' : ' . __LINE__, "Connexion TLS échouée, envoi des données impossible sur : " . $this->host);
        $socket = fsockopen('ssl://' . $this->host, $this->port, $this->errno, $this->errstr, CertissimSocket::TIMEOUT);
    if($socket == false){
        insertLogCertissim(__METHOD__ . ' : ' . __LINE__, "Connexion SSL échouée, envoi des données impossible sur : " . $this->host);
    }
    }
    }
    else
    ...
    if ($socket !== false) {
      ...
      //if header sending is impossible : log
        ...
        insertLogCertissim(__METHOD__ . ' : ' . __LINE__, "Envoi des donnÃ©es impossible sur : " . $host);
        $res = false;
      }
      //closes the connexion
      fclose($socket);
    } else { //if connection failed, log
      insertLogCertissim(__METHOD__ . ' : ' . __LINE__, "Connexion socket impossible sur l'hÃ´te $host. Erreur " . $this->errno . " : " . $this->errstr);
      $res = false;
    }
